...
- 覆盖：绑定验证器、开锁失败锁定、CRUD 权限、只读分享、未开锁不能 reveal、未开锁可改标题描述且密文不变、未开锁带 fields 被拒、已有记录改分类被拒、新分类校验、自己改显示名、更换验证器、签发恢复码、恢复码一次性消费、恢复码与 TOTP 共用锁定、reset 接受恢复码、重新生成作废旧码、生产 Access JWT 身份、本地 `DEV_TOTP_BYPASS` 仅非生产对 `000000` 生效、审计分页每页 10 条
