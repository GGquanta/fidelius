...
折页是挂在柜面顶边上的标签，不是拉满宽度的分段控件。

- 每页固定 88×88，等高、等宽，禁止因激活而加高或加 padding
- 左对齐，页与页间隙 4px，相对柜面左右各留 12px；装不下时横向滚动，不换行、不伸缩
- 每页：32px 瓷砖、短名、计数，纵向居中，间隙 4px
- 层叠：未激活 `z-index: 0`，纸张 `z-index: 1` 且 `margin-top: -1px` 压住页脚，激活页 `z-index: 2`、底为 surface，用同色盖住纸张顶边。禁止身后假纸边，禁止激活页改变高度
- 纸张圆角 12 / 12 / 18 / 18，只有一层面 `--elev-3`，`min-height: 480px`
- 折页内的记录卡只用边框与 canvas 底，禁止再加阴影
